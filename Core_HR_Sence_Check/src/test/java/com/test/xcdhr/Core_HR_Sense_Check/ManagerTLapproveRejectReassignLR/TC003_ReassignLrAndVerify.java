...
public class TC003_ReassignLrAndVerify extends TestBase {
	...
	public void ClickLeaveTabAndRecord() throws InterruptedException{
		...
		if(verifyLrRecord){
			leaveApproveRejectReassign.clickLeaveRequestRecordForReassign();
		}
	}
	@Test(dependsOnMethods ={"ClickLeaveTabAndRecord"}) 
	public void verifyApproveRejectAndReassign() throws InterruptedException{
	
		boolean reassignLink = leaveApproveRejectReassign.verifyReassignLink();
		Assert.assertEquals(reassignLink, true);
		log.info("--------"+reassignLink);
		if(reassignLink){
			leaveApproveRejectReassign.clickReassignLink();
		}
		leaveApproveRejectReassign.reassignToOtherManager();
		
	}	
	
/*	@Test (dependsOnMethods = {"verifyApproveRejectAndReassign"})
	public void clickReassigApprovalRequest(){
		boolean reassignApprovalReqButton = leaveApproveRejectReassign.verifyReassignApprovalButton();*/
		
	/*	log.info("902384409823"+reassignApprovalReqButton);
		if(reassignApprovalReqButton){
			leaveApproveRejectReassign.clickReassignApprovalButton();*/
		}
