...
public class TC003_VerifyAddPaymentOrDeductionOneOffPaymentType extends TestBase{
AddPaymentOrDeductionPage addPaymentDeduction; 
	
	public static final Logger log = Logger
			.getLogger(TC003_VerifyAddPaymentOrDeductionOneOffPaymentType.class.getName());
	...
	public void verifyPaymentDedutonAndClick() throws InterruptedException {
		...
		boolean verifyPaymentDeduction = addPaymentDeduction.verifyPaymentDeductionRecordForOneOffPayment();
		log.info("Payment deduction record is displayed =====" + verifyPaymentDeduction);
		Assert.assertEquals(verifyPaymentDeduction, true);
		if (verifyPaymentDeduction) {
			addPaymentDeduction.clickpaymentDeductionRecordForOneOffPayment();
	...
	public void verifyPaymentAmount(String verifyPaymentAmount) {
		...
		String actualPaymentAmount = addPaymentDeduction.paymentAmountMatchesForOneOffPayment();
	...
	public void verifyRecordType(String verifyRecordType) {
		...
		String actualRecordType = addPaymentDeduction.recordTypeMatchesForOneOffPayment();
		log.info("-------"+expectedRecordType);
